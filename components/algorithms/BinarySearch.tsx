      ...
      <div className="array-container">
        <div className="array">
          {sortedArray.map((num, idx) => {
            ...
            if (step) {
              if (idx === step.mid && step.status === 'found') className += ' found'
              else if (idx === step.mid) className += ' current'
              else if (idx >= step.left && idx <= step.right) className += ' range'
      ...
      <div className="steps">
        {steps.map((step, idx) => (
          <div key={idx} className="step">
            Step {idx + 1}: L={step.left}, R={step.right}, Mid={step.mid} ({sortedArray[step.mid]})
            {step.status === 'found' && ' ✓'}
          </div>
        ))}
      </div>
