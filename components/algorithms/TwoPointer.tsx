...
export default function TwoPointer() {
  ...
  const sortedArray = [5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75]
  ...
  return (
    <div className="algo-container">
      <div className="controls">
        ...
        <button onClick={() => { setSteps([]); setResult('') }}>Reset</button>
      </div>
      ...
      <div className="array-container">
        <div className="array">
          {sortedArray.map((num, idx) => {
            ...
            if (step) {
              if ((idx === step.left || idx === step.right) && step.status === 'found')
                className += ' found'
              else if (idx === step.left || idx === step.right) className += ' pointer'
      ...
      <div className="steps">
        {steps.map((step, idx) => (
          <div key={idx} className="step">
            Step {idx + 1}: L={step.left} ({sortedArray[step.left]}) + R={step.right} (
            {sortedArray[step.right]}) = {step.sum}
            {step.status === 'found' && ' ✓'}
          </div>
        ))}
      </div>
