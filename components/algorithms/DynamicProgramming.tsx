...
export default function DynamicProgramming() {
  ...
  const [dp, setDp] = useState<FibStep[]>([])
  ...
  const calculateFibonacci = useCallback(async () => {
    ...
    const fibArray: FibStep[] = Array.from({ length: n + 1 }, (_, i) => ({
      index: i,
      value: 0,
      calculated: false,
    }))
  ...
  return (
    <div className="algo-container">
      <div className="controls">
        ...
        <button onClick={() => { setDp([]); setResult(null) }}>Reset</button>
      </div>
        ...
        <div className="dp-table">
          {dp.map((item) => (
            ...
            >
              ...
              <div className="dp-value">{item.calculated ? item.value : '?'}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="explanation">
        <p>
          Dynamic Programming builds solutions bottom-up. Each value depends on previously
          calculated values: F(n) = F(n-1) + F(n-2)
        </p>
      </div>
    </div>
  )
